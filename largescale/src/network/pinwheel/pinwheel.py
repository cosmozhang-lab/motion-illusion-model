...
class PinwheelNetwork:
  def __init__(self, config = CommonConfig()):
    ...
    config.amp_lgn_on_pos = config.fetch("amp_lgn_on_pos", 0.0)
    config.tau_rise_lgn_on_pos = config.fetch("tau_rise_lgn_on_pos", 0.0)
    config.tau_damp_lgn_on_pos = config.fetch("tau_damp_lgn_on_pos", 0.0)
    config.lgn_ikernels_on = config.fetch("lgn_ikernels_on", None)
    config.amp_lgn_on_neg = config.fetch("amp_lgn_on_neg", 0.0)
    config.tau_rise_lgn_on_neg = config.fetch("tau_rise_lgn_on_neg", 0.0)
    config.tau_damp_lgn_on_neg = config.fetch("tau_damp_lgn_on_neg", 0.0)
    config.lgn_kernels_off = config.fetch("lgn_kernels_off", None)
    config.amp_lgn_off_pos = config.fetch("amp_lgn_off_pos", 0.0)
    config.tau_rise_lgn_off_pos = config.fetch("tau_rise_lgn_off_pos", 0.0)
    config.tau_damp_lgn_off_pos = config.fetch("tau_damp_lgn_off_pos", 0.0)
    config.lgn_ikernels_off = config.fetch("lgn_ikernels_off", None)
    config.amp_lgn_off_neg = config.fetch("amp_lgn_off_neg", 0.0)
    config.tau_rise_lgn_off_neg = config.fetch("tau_rise_lgn_off_neg", 0.0)
    config.tau_damp_lgn_off_neg = config.fetch("tau_damp_lgn_off_neg", 0.0)
